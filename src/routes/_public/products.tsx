...
export function enhanceProductWithNanoBanana(p: any) {
  ...
  const isKeyboard = nameLower.includes("keyboard") || nameLower.includes("keypad") || catLower.includes("keyboard");
  const isMouse = nameLower.includes("mouse") || nameLower.includes("mice") || nameLower.includes("trackball") || catLower.includes("mouse");
  const isAudio = nameLower.includes("headset") || nameLower.includes("speaker") || nameLower.includes("audio") || nameLower.includes("pebble") || nameLower.includes("jbl") || nameLower.includes("sound") || nameLower.includes("headphones");
  const isMic = nameLower.includes("mic") || nameLower.includes("microphone") || nameLower.includes("quadcast") || nameLower.includes("yeti") || nameLower.includes("shure") || nameLower.includes("rode");
  const isWebcam = nameLower.includes("webcam") || nameLower.includes("camera") || nameLower.includes("brio") || nameLower.includes("kiyo") || nameLower.includes("insta360");
  const isDisplay = nameLower.includes("monitor") || nameLower.includes("display") || nameLower.includes("ultrawide") || nameLower.includes("screen") || nameLower.includes("crt");
  const isPrinter = nameLower.includes("printer") || nameLower.includes("ecotank") || nameLower.includes("laserjet") || nameLower.includes("pixma") || nameLower.includes("inkjet") || nameLower.includes("writer") || nameLower.includes("brother");
  const isScanner = nameLower.includes("scanner") || nameLower.includes("scan") || nameLower.includes("scansnap");
  const isTablet = nameLower.includes("pencil") || nameLower.includes("stylus") || nameLower.includes("tablet") || nameLower.includes("digitizer") || nameLower.includes("wacom") || nameLower.includes("pen");
  const isPower = nameLower.includes("ups") || nameLower.includes("battery") || nameLower.includes("power") || nameLower.includes("apc") || nameLower.includes("charge");
  const isStorage = nameLower.includes("ssd") || nameLower.includes("hdd") || nameLower.includes("drive") || nameLower.includes("flash") || nameLower.includes("memory") || nameLower.includes("storage");
  const isProjector = nameLower.includes("projector") || nameLower.includes("optoma") || nameLower.includes("tk700");
  const isVr = nameLower.includes("vr") || nameLower.includes("virtual reality") || nameLower.includes("oculus") || nameLower.includes("quest");
  const isPresenter = nameLower.includes("presenter") || nameLower.includes("remote") || nameLower.includes("pointer");
  const isDongle = nameLower.includes("adapter") || nameLower.includes("dongle") || nameLower.includes("wifi") || nameLower.includes("bluetooth") || nameLower.includes("hub") || nameLower.includes("dock") || nameLower.includes("link");
  ...
  let boxContents = ["Nano Banana Hardware Unit", "1.8m Braided USB-C Cable", "Calibration Guide", "Academic Warranty Card"];
  ...
  if (isKeyboard) {
    ...
    pros = ["Crisp tactile sound feedback", "Lubricated stabilizers out of the box", "Heavy frame reduces sliding"];
    cons = ["No dedicated numpad", "Custom keycap stems are tight"];
    boxContents = ["TKL Mechanical Keyboard", "USB-C to USB-A Gold plated cable", "Keycap & Switch puller tool", "3 Extra Banana Switch replacements"];
  } else if (isMouse) {
    ...
    pros = ["Sub-millisecond tracking delay", "Extremely lightweight structure", "Wired/Wireless dual connectivity"];
    cons = ["Requires dongle extension for 8000Hz stability", "Large footprint not suitable for tiny hands"];
    boxContents = ["Wireless Precision Mouse", "2.4GHz USB Dongle Receiver", "USB-C paracord charging cable", "PTFE mouse feet replacements"];
  } else if (isMic) {
    ...
    pros = ["Vocal broadcast richness", "Four adjustable recording patterns", "Sturdy metal desk stand"];
    cons = ["Highly sensitive to ambient typing noise", "Large heavy mic assembly"];
    boxContents = ["Studio Broadcaster Microphone", "Solid metal desktop stand", "2m USB-C braided cable", "Anti-dust microphone pop filter"];
  } else if (isWebcam) {
    ...
    pros = ["Extremely sharp 4K resolution", "Excellent performance in dim laboratory lighting", "Dynamic AI auto-framing"],
    cons = ["No manual focus ring override", "Gets warm during extended 4K broadcasts"];
    boxContents = ["4K Live Stream Webcam", "Removable privacy shutter cap", "USB-C to USB-C cable (1.5m)", "Carrying protective sleeve"];
  } else if (isAudio) {
    ...
    pros = ["Superb spatial audio positioning", "Thick memory foam ear cushions", "Clear mic broadcast quality"];
    cons = ["Headphones sit tight initially", "Requires software client for virtual 7.1 mapping"];
    boxContents = ["Studio Wireless Headset", "Detachable Broadcast Microphone", "3.5mm Audio Cable", "Soft Travel Pouch"];
  } else if (isDisplay) {
    ...
    pros = ["Immense workspace desktop real-estate", "Vibrant colors for creative editing", "USB-C charge laptop directly"],
    cons = ["Heavy stand requires sturdy desk support", "Large box dimensions require freight dispatch"];
    boxContents = ["34\" IPS curved monitor", "Heavy-duty height-adjustable stand", "DisplayPort 1.4 Cable", "USB-C Cable (E-Marker 100W)"];
  } else if (isPrinter) {
    ...
    pros = ["Extremely low cost per printed page", "Refillable ink tanks are mess-free", "Fast first-page print out time"];
    cons = ["Heavy footprint for desktop placement", "Scanner feeder lacks auto-duplex"];
    boxContents = ["All-in-one printer unit", "4-bottle ink set (CMYK)", "Power Cable", "Installation setup CD & USB cable"];
  } else if (isScanner) {
    ...
    pros = ["Ultra-fast decode rate", "Rugged shock-absorbent body", "Wireless Bluetooth link up to 10m"];
    cons = ["Heavier than basic laser scanners", "Acoustic beep cannot be disabled"];
    boxContents = ["Handheld Area Imager", "Charging Cradle Base", "2m Coiled USB Connection Cable", "User configuration sheet"];
  } else if (isTablet) {
    ...
    pros = ["Highly natural writing feeling", "Battery-free stylus requires zero charging", "Vast active drawing area"];
    cons = ["Pen nibs wear down with high usage", "Tablet driver setup is complex"];
    boxContents = ["Digitalizer Drawing Tablet", "Battery-free Stylus Pen", "Pen Stand holder", "8 Replacement drawing nibs"];
  } else if (isPower) {
    ...
    pros = ["Highly reliable server backup", "Surge protection prevents hardware damage", "Alert sounds can be muted"],
    cons = ["Extremely heavy lead-acid battery unit", "Battery needs recycling replace every 3 years"];
    boxContents = ["Smart-UPS Battery Backup", "USB Communications Cable", "Hardware mounting bracket", "User manual log"];
  } else if (isStorage) {
    ...
    pros = ["Pocket-sized form factor", "Extremely fast file transfers", "IP65 water and dust resistance"];
    cons = ["Gets slightly warm during massive reads", "No integrated cable storage wrap"];
    boxContents = ["External Portable SSD", "USB Type-C to C cable", "USB Type-C to A adapter cable", "Warranty pamphlet"];
  } else if (isProjector) {
    ...
    pros = ["Cinematic color accuracy", "High brightness fits daytime labs", "Low input delay for smooth presentation scrolling"],
    cons = ["Cooling fan produces audible hum", "Replacement lamp cost is substantial"];
    boxContents = ["Smart Projector Unit", "Backlit Remote Control (with batteries)", "1.8m Power Cable", "Quick Start Guide"];
  } else if (isVr) {
    ...
    pros = ["Completely standalone - no PC required", "Immersive 3D audio experience", "Extensive productivity software selection"];
    cons = ["Battery life is limited to 2 hours", "Stock headstrap can cause facial pressure"];
    boxContents = ["VR Headset Unit", "Two Touch Wireless Controllers", "Silicone Facial Interface Cover", "USB-C charging adapter & cable"];
  } else if (isPresenter) {
    ...
    boxContents = ["Wireless Presenter Remote", "Mini USB Receiver", "1x AAA Alkaline Battery", "User reference guide"];
  } else if (isDongle) {
    ...
    pros = ["Lightweight portable structure", "Sleek metallic heat dissipation", "No external power source required"];
